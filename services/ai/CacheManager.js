...
class CacheManager {
        ...
        generateHash(prompt, language = "auto", model = "groq") {
                const data = `${prompt}:${language}:${model}`;
                return crypto.createHash("sha256").update(data).digest("hex");
        }

        async get(prompt, language = "auto", model = "groq") {
        ...
        async set(prompt, response, language = "auto", model = "groq", ttl = null) {
                const DataBase = useDB();
                const promptHash = this.generateHash(prompt, language, model);
        ...
        async invalidate(prompt, language = "auto", model = "groq") {
                const DataBase = useDB();
                const promptHash = this.generateHash(prompt, language, model);
