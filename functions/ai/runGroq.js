        ...
        try {
                ...
                if (!apiKey) {
                        return await interaction.editReply({
                                content: "❌ **Groq AI chưa được cấu hình!**\n\n🔑 Admin cần thêm `GROQ_API_KEY` vào Secrets để sử dụng tính năng này.\n\n📖 Hướng dẫn lấy API key:\n• Truy cập: https://console.groq.com/keys\n• Tạo tài khoản miễn phí\n• Copy API key\n• Thêm vào Secrets với tên `GROQ_API_KEY`",
                        });
                }
                
                if (!AIService.groqService) {
                        return await interaction.editReply({
                                content: "❌ **Groq service chưa khởi tạo!**\n\nVui lòng:\n• Kiểm tra API key trong Secrets\n• Restart bot sau khi thêm key\n• Liên hệ admin nếu vấn đề vẫn tiếp diễn",
                        });
                }
                ...
                await interaction.editReply({
                        content: "🤔 **Đang suy nghĩ...**\n*Groq AI đang xử lý câu hỏi của bạn*",
                });
                ...
                const createButtons = (page, disableFeedback = false) => {
                        ...
                        const feedbackRow = new ActionRowBuilder().addComponents(
                                new ButtonBuilder()
                                        .setCustomId(`groq_feedback_positive_${interaction.id}`)
                                        .setEmoji("👍")
                                        .setLabel("Hữu ích")
                                        .setStyle(ButtonStyle.Success)
                                        .setDisabled(disableFeedback),
                                new ButtonBuilder()
                                        .setCustomId(`groq_feedback_negative_${interaction.id}`)
                                        .setEmoji("👎")
                ...
                if (result.generatedImage) {
                        try {
                                ...
                                console.log("[runGroq] ✅ Attached generated image to response");
                        } catch (imageError) {
                                console.error("[runGroq] Error attaching image:", imageError);
                        }
                }
                ...
                collector.on("collect", async (btnInteraction) => {
                        if (!btnInteraction.customId.startsWith("groq_feedback")) {
                        ...
                        } else if (btnInteraction.customId.startsWith("groq_feedback")) {
                                if (feedbackGiven) {
                                        return await btnInteraction.reply({
                ...
                collector.on("end", async () => {
                        try {
                                ...
                                console.error("[runGroq] Error disabling buttons:", error);
                        }
                });
