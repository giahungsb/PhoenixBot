...
const initialize = async () => {
        ...
        const Database = require("@zibot/db");
        
        const mongoConnected = await mongoose.connect(process.env.MONGO).catch(() => false);
        ...
        if (mongoConnected) {
                ...
                logger.error("Failed to connect to MongoDB! Using local fallback database.");
                const db = new Database("./jsons/ziDB.json");
                const createModel = (db, name) => ({
                        findOne: (query) => db.get(`${name}.${JSON.stringify(query)}`),
                        find: () => db.all(name),
                        create: (data) => db.set(`${name}.${data.guildId || data.userID || Date.now()}`, data),
                        updateOne: (query, update) => {
                                const key = `${name}.${JSON.stringify(query)}`;
                                const existing = db.get(key) || {};
                                db.set(key, { ...existing, ...update.$set });
                        },
                        deleteOne: (query) => db.delete(`${name}.${JSON.stringify(query)}`),
                });
                ...
                useDB({
                        ...
                        ZiGoldPrice: createModel(db, "ZiGoldPrice"),
                        ZiGroqThread: createModel(db, "ZiGroqThread"),
                        ZiFeedback: createModel(db, "ZiFeedback"),
                        ZiAnswerCache: createModel(db, "ZiAnswerCache"),
                        ZiLog: createModel(db, "ZiLog"),
                        ZiAIRoom: createModel(db, "ZiAIRoom"),
                });
        }
