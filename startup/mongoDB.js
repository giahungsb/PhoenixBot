...
const ZiUser = Schema({
        ...
        // AI system
        groqHistory: { type: Array, default: [] },
...
const ZiGuild = Schema({
        ...
        aiRooms: {
                enabled: { type: Boolean, default: false },
                categoryId: { type: String, default: null },
                maxRoomsPerGuild: { type: Number, default: 10 },
                inactivityTimeout: { type: Number, default: 900000 },
                allowPublicView: { type: Boolean, default: false },
        },
});

ZiGuild.index({ guildId: 1 }, { unique: true });
...
const ZiAIRoom = Schema(
        {
                guildId: { type: String, required: true },
                userId: { type: String, required: true },
                channelId: { type: String, required: true, unique: true },
                threadId: { type: String, required: true },
                aiModel: { type: String, enum: ["ask", "groq", "gpt5", "gpt5-mini", "claude", "gemini-pro", "gemini-flash"], default: "ask" },
                createdAt: { type: Date, default: Date.now },
...
ZiAIRoom.index({ guildId: 1, userId: 1 });
ZiAIRoom.index({ guildId: 1, userId: 1, aiModel: 1, status: 1 }, { unique: true, partialFilterExpression: { status: "active" } });
ZiAIRoom.index({ status: 1, cleanupScheduledAt: 1, lastActivity: 1 });
